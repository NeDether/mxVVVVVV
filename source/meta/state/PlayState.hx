...
class PlayState extends MusicBeatState
{
	...
	public static var isMX:Bool = false;
	public static var isChase:Bool = false;
	...
	override public function create()
	{
		...
		add(dadOpponent);
		add(stageBuild.backgroundsArray['wall']);
		add(boyfriend);
		...
		dadStrums = new Strumline(placement - (FlxG.width / 4) + offset, this, dadOpponent, false, true, false, 4, Init.trueSettings.get('Downscroll'));
		dadStrums.visible = !Init.trueSettings.get('Centered Notefield');
		boyfriendStrums = new Strumline(placement + (!Init.trueSettings.get('Centered Notefield') ? (FlxG.width / 4) : 0) - offset, this, boyfriend, true, false, true,
		...
		if (!inCutscene) {
			...
			if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
			{
				...
				if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
				{
					...
					{
						case 'pcport':
							getCenterX = Std.int(boyfriend.getMidpoint().x - (36 * 6));
							getCenterY = Std.int(boyfriend.getMidpoint().y - (24 * 6));
					}

					camFollow.setPosition(128, 128);
				...
				else
				{
					...
					{
						case 'pcport':
							getCenterX = Std.int(char.getMidpoint().x - (36 * 6));
							getCenterY = Std.int(char.getMidpoint().y - (24 * 6));
					}
	...
	function powerupVisuals(animation:String)
	{
		...
		if (prefix == "bf-glitch" || prefix == "bf-smm" || prefix == 'bf-flipped')
		{
			prefixMatches = true;
		}
	...
	override function stepHit()
	{
		...
		if (isMX)
		{
			...
			if (curStep == 507)
			{
				...
				dadOpponent.stopAnim = true;
			}
			
			if (curStep == 751)
			{
				dadOpponent.playAnim('laugh1', true);
				dadOpponent.stopAnim = true;
			}
			...
			{
				dadOpponent.stopAnim = false;
				dadOpponent.playAnim('laugh2', true);
				dadOpponent.stopAnim = true;
			}
			...
			{
				dadOpponent.stopAnim = false;
				dadOpponent.playAnim('laugh3', true);
				dadOpponent.stopAnim = true;
			}
			...
			if (curStep == 769)
			{
				...
				stageBuild.bfPos = new FlxPoint(boyfriend.x, boyfriend.y);
				stageBuild.bfDefPos = new FlxPoint(boyfriend.x, boyfriend.y);
				stageBuild.bfTarget = new FlxPoint(boyfriend.x + (64 * 6), boyfriend.y - (50 * 6));
				isChase = true;
			}
	...
	override function beatHit()
	{
		...
		if (SONG.song.toLowerCase() == "game-over")
		{
			//lol
		}
